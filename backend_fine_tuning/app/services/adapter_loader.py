...
def _download_dataset_adapter(config: AdapterConfig) -> Path:
    ...
    # Resolve checkpoints relative to backend_fine_tuning/fine_tuning/checkpoints/
    # regardless of where this file lives inside the package.
    local_dir = Path(__file__).resolve().parents[2] / "fine_tuning" / "checkpoints" / config.adapter_repo_id.replace("/", "__")
    allow_patterns = [
